...
export function MusingItem({ title, time, active = false, onClick, onDelete }: MusingItemProps) {
  return (
    <div 
      onClick={onClick}
      className={cn(
    ...
    >
      <div className="flex items-center gap-3 truncate pr-4">
        <div className={cn(
          ...
          active ? "bg-accent scale-125 shadow-[0_0_8px_rgba(var(--accent),0.8)]" : "bg-text-secondary/20"
        )} />
        <span className={cn(
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
            <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/5">
              <MoreHorizontal className="w-3.5 h-3.5 text-text-secondary" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-40 bg-surface-card border-border-subtle p-1 rounded-xl shadow-xl">
            <DropdownMenuItem 
              onClick={(e) => {
                e.stopPropagation();
                onDelete?.();
      ...
      <aside className="hidden xl:flex w-84 h-full border-l border-border-subtle bg-surface-card/30 backdrop-blur-sm shrink-0 flex-col overflow-hidden">
        ...
        <div className="p-8 pb-4 shrink-0">
          <div className="flex items-center justify-between mb-8">
            <h2 className="text-[11px] font-bold uppercase tracking-[0.2em] text-text-secondary">Recent Chats</h2>
            <Button 
              onClick={onNewChat}
              variant="outline" 
              size="icon" 
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1.5 no-scrollbar">
          {isLoading ? (
           <div className="px-6 py-12 text-center">
            ...
            <div className="px-6 py-12 text-center">
              <p className="text-[10px] font-bold text-text-secondary/40 uppercase tracking-widest leading-relaxed">
                {searchTerm ? "No matches found." : "No sessions found."}<br/>
            ...
            <>
              <div className="mb-4 px-4 pt-2">
                <span className="text-[9px] font-black text-text-secondary/30 uppercase tracking-[0.2em]">Active Conversations</span>
              </div>
